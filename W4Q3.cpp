...
//  second prong,
    if (T == 004) {     // prong 2, for function handling tower inputs func.temp
        cin >> P;
        if (P == "T1" || P == "1" || P == "tower 1" || P == "t1") return '1';
            else if (P == "T2" || P == "2" || P == "tower 2" || P == "t2") return '2';
                else if (P == "T3" || P == "3" || P == "tower 3" || P == "t3") return '3';
...
void Print(vector<char> T1C_vector,char &ls01) {
    ...
    if (ls01 == '1') {                   // seocond prong
        ...
        for (int i = 0; i < T1C_vector.size(); i++ ) {            // to print the board with letters on left side
            cout << "  " << T1C_vector[i] << "    _    _\n";
            if ((i+1) == T1C_vector.size()) { cout << " T01  T02  T03 \n\n"; }
        }
...
// check if source tower is empty
            if (U == '1'){  if (T1C_vector.size() > 1) { T01 = T1C_vector.front(); break; }
                    else cout << "tower is empty enter a new source\n";}
            if (U == '2'){  if (T2C_vector.size() > 1) { T01 = T2C_vector.front(); break; }
                    else cout << "tower is empty enter a new source\n";}
            if (U == '3'){  if (T3C_vector.size() > 1) { T01 = T3C_vector.front(); break; }
                    else cout << "tower is empty enter a new source\n"; }
    }   }
    if (W == 1) {
        while (ls01) {
...
// check if the destination tower will still be in alphabetical order, if not deny request
            if (V == '1') {
                if (static_cast <int>(T1C_vector.back()) < static_cast <int> (T01))
                    cout << "tower would no longer be in alphabetical order, put in new input"; } else {break;}
            if (V == '2') {
                if (static_cast <int>(T2C_vector.back()) < static_cast <int> (T01))
                    cout << "tower would no longer be in alphabetical order, put in new input"; } else {break;}
            if (V == '3') {
                if (static_cast <int>(T3C_vector.back()) < static_cast <int> (T01))
                    cout << "tower would no longer be in alphabetical order, put in new input"; } else {break;}
        }
    }
...
void LT_choice(vector<char> &T1C_vector, vector <char> &T2C_vector, vector <char> &T3C_vector) {
    ...
    int W; char T01,V,U,SW;
    cout << "what tower should a letter be moved from :";
        temp(T1C_vector,T2C_vector,T3C_vector,0,T01,V,U);   //  get starting tower uses prong indicator W

    cout << "what tower should the letter be moved to :";
...
// ask to confirm the transfer
    ...
    if (SW == 'y') {
        if (U == '1')
            if (V == '2') { T2C_vector.push_back(T1C_vector.front()); }     // transfer T1 to T2
            if (V == '3') { T3C_vector.push_back(T1C_vector.front()); }     // transfer T1 to T3

        if (U == '2')
            if (V == '1') { T1C_vector.push_back(T2C_vector.front()); }     // transfer T2 to T1
            if (V == '3') { T3C_vector.push_back(T2C_vector.front()); }     // transfer T2 to T3

        if (U == '3')
            if (V == '1') { T1C_vector.push_back(T3C_vector.front()); }     // transfer T3 to T1
            if (V == '2') { T2C_vector.push_back(T3C_vector.front()); }     // transfer T3 to T2
    }


    for (char n:T1C_vector) {cout << n << endl;}
    for (char n:T2C_vector) {cout << n << endl;}
    for (char n:T3C_vector) {cout << n << endl;}
}
...
//    print board
    ...
    for (int i = 0; i < T1C_vector.size(); i++) {

        for (int i = 0; i < 3; i++) {
            cout << T1C_vector[1] << "   " << T2C_vector[i] << "    " << T3C_vector[i] << endl;
        }
    }
    cout << " T01  T02  T03 \n";
...
int main () {
    ...
    bool ls1 = 1;
    int SC;                   // does not need to be in loop as it is pass by value
...
//      display updated board
            print_U(T1C_vector,T2C_vector,T3C_vector);
